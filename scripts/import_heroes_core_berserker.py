#!/usr/bin/env python3
"""Import Berserker data from Heroes Core reference payload into SQLite.

This importer is intentionally scoped to one class (Berserker) and verifies
key anchors against the Heroes Core Book PDF before writing data.
"""
...
def collect_anchors(reference: dict[str, Any]) -> list[str]:
    anchors = [
        "The Berserker is destruction.",
        "Key Stats: STR, DEX",
        "Starting HP: 20",
        "Saves: STR+, INT",
        "Savage Arsenal",
    ]
    anchors.append(reference["class"]["name"])
...
def upsert_berserker(conn: sqlite3.Connection, reference: dict[str, Any], ruleset_id: int) -> dict[str, int]:
    cls = reference["class"]
    row = conn.execute(
    ...
    for row in reference["progression"]:
        ...
        conn.execute(
            ...
            INSERT INTO class_level_feature (
                ruleset_id, class_id, subclass_id, level, name, description, feature_type, display_order
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ruleset_id,
    ...
    for row in reference["choiceOptions"]:
        ...
        conn.execute(
            ...
            INSERT INTO feature_choice_option (
                choice_group_id, name, description, prereq_json, effects_json, display_order
            ) VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                group_id,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import Berserker data from Heroes Core reference payload")
...
def main() -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        stats = upsert_berserker(conn, reference, ruleset_id)
        conn.commit()

    print(
        "Imported Berserker from Heroes Core reference "
